circ.regs <- function(y, x, rads = TRUE, type = "vm", tol = 1e-6, logged = FALSE, maxiters = 100, ncores = 1) {
  ...
  if ( type == "pn" ) {
    ...
    if ( type == "vm" ) {
      ...
      lik0 <- Rfast::purka.mle(y, tol = tol)$loglik
      lik <- .cp.regs(y = y, x = x)
      stat <- 2 * (lik - lik0)
    } else if ( type == "cipc" ) {
...
.vm.regs <- function(y = y, x = x, tol = tol, maxiters = maxiters, ncores = 1) {
  ...
  if ( ncores <= 1 ) {
    for ( j in 1:p )  stat[j] <- .vm.reg(y, x, tol = tol, maxiters = maxiters)$loglik
  } else {
    ...
    stat <- parallel::parSapply(cl, 1:p, function(j) {
      .vm.reg(y, x, tol = tol, maxiters = maxiters)$loglik
    })
...
.cp.regs <- function(y = y, x = x, tol = tol, maxiters = maxiters, ncores = 1) {
  ...
  if ( ncores <= 1 ) {
    for ( j in 1:p )  stat[j] <- Directional::circpurka.reg(y, x, tol = tol, maxiters = maxiters)$loglik
  } else {
    cl <- parallel::makeCluster(ncores)
